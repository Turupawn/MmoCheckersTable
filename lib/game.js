...
/*
var a = https.createServer(options,ecstatic({ root: path.resolve(__dirname, '../public') }), function (req, res) {
  res.writeHead(200);
  res.end("hello world\n");
  init()
}).listen(3000);
...
function onNewPlayer (data) {
  ...
  for(var i=0;i<objects_crowned.length;i++)
  {
    this.emit('crown', { index: i, crowned: objects_crowned[i]})    
  }
...
function onMoveSharedObject (data) {
  ...
  if(data.crown)
  {
    new_crowned = !objects_crowned[data.index]
    objects_crowned[data.index]=new_crowned
    this.broadcast.emit('crown', { index: data.index, crowned: new_crowned })
  }
}
